...
// Declare obs_data_array_t* as an opaque pointer type for Qt's meta-object system
				...
				obs_data_t *response_data, void *priv_data)
{
	...
	blog(LOG_INFO, "[obs-ptz-websocket] ptz_stop -> %s",
			       ...
			       void *priv_data)
{
	...
	blog(LOG_INFO, "[obs-ptz-websocket] ptz_get_presets -> %s",
	     success ? "success" : "failed");
}
...
void ptz_load_websocket(void)
{
	...
	// Register custom types with Qt's meta-object system for cross-thread invocation
	qRegisterMetaType<obs_data_array_t*>("obs_data_array_t*");
